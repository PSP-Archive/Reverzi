...
int FontHeight(){
	return FontHeight2(&InternalFont);
}
...
Uint32 GetPixel(SDL_Surface *Surface, Sint32 X, Sint32 Y)
{
   ...
   if (X<0) puts("SFONT ERROR: x too small in GetPixel.");
   if (X>=Surface->w) puts("SFONT ERROR: x too big in GetPixel.");
...
//		    printf("printing %c %d\n",text[i],ofs);
            ...
            srcrect.y = 1;
    	    dstrect.x = (short)(x-(float)(Font->CharPos[ofs]-Font->CharPos[ofs-1])/2);
			dstrect.y = y;
...
//		printf("%s ## %d\n",text,strlen(text));
	...
	if (SDL_GetTicks()>(unsigned)blinktimer) {
	    blink=1-blink;
